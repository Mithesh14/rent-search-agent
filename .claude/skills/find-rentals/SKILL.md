---
name: find-rentals
description: Run the rent search — fetch new listings from NoBroker, OLX, MagicBricks, CommonFloor, and SquareYards, hard-filter, view each listing's photo, score against preferences.md, and produce today's shortlist. Use when the user runs /find-rentals.
---
...
1. Run `PYTHONPATH=src python3 -m rentsearch.fetch`. This fetches from NoBroker, OLX,
   MagicBricks, CommonFloor, and SquareYards (all native adapters — no MCP server or staging
   file needed), dedupes, stores
...
4. For each unscored listing:
   - If it has an `image_url`, download it and actually view it with the Read tool before
     ...
     `Referer: https://www.nobroker.in/` header or it 403s; OLX, MagicBricks, CommonFloor, and
     SquareYards images don't: `curl -sL -H "Referer: https://www.nobroker.in/" -o /tmp/listing_<id>.jpg "<image_url>"`.
